...
const NAV: { id: Section; label: string; glyph: string; ready: boolean }[] = [
  { id: "fleet", label: "Fleet", glyph: "◍", ready: true },
  { id: "board", label: "Board", glyph: "▤", ready: true },
  { id: "sessions", label: "Sessions", glyph: "▦", ready: true },
  { id: "admin", label: "Admin", glyph: "⚙", ready: true },
...
export function Sidebar({
  ...
  return (
    <aside className="glass flex w-full shrink-0 items-center gap-1 overflow-x-auto border-x-0 border-t-0 px-3 py-3 md:w-[224px] md:flex-col md:items-stretch md:overflow-visible md:border-y-0 md:border-l-0 md:px-4 md:py-5">
      <div className="mr-2 flex shrink-0 items-center gap-2 px-2 md:mb-6 md:mr-0">
        <div className="grid h-8 w-8 place-items-center rounded-lg bg-[var(--color-accent)]/15 text-[var(--color-accent-2)]">
          ⬡
        </div>
      ...
      {NAV.map((item) => {
        ...
        return (
          <button
            ...
            className={`relative flex h-10 w-10 shrink-0 items-center justify-center gap-3 rounded-lg text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]/60 sm:w-auto sm:justify-start sm:px-3 sm:py-2 ${
              active
                ? "bg-[var(--color-accent)]/12 text-[var(--color-ink)]"
                : "text-[var(--color-muted)] hover:bg-white/[0.03] hover:text-[var(--color-ink)]"
            }`}
          >
            <span className="w-4 text-center text-[var(--color-accent-2)]">{item.glyph}</span>
            ...
            {active && item.ready && (
              <motion.span
                layoutId="nav-dot"
                className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-[var(--color-accent-2)] sm:static sm:ml-auto"
              />
            )}
          </button>
        );
